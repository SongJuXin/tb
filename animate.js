...
(function(){
	function animate(obj,target,opt){
		var opt=opt||{}
		var change={}
		var begin={}
		var time=0
		var duration=opt.duration||1000
		var effect=opt.effect||tween.Linear
		var callback=opt.callback
		//efect相关
		//1.var ary = ["Linear", "Elastic-easeOut", "Back-easeOut", "Bounce-easeOut", "Expo-easeIn"]
		if(typeof effect=='number'){
			effect=ary[effect%ary.length].split('-')
			effect=effect.length>=2?tween[effect[0]][effect[1]]:tween[effect[0]]
		}
		//2['Elastic','easeOut']
			...
			effect=effect.split('.')
			effect=effect=effect.length>=2?tween[effect[0]][effect[1]]:tween[effect[0]]
		}
